...
def find_optimal_k(df, features):
    inertia = []
    k_range = range(2, 10)  # 测试从 2 到 10 个簇
    for k in k_range:
    ...
    plt.show()
    # plt.savefig("elbow_no_timeslot.png", format='png', bbox_inches='tight')
    # 返回肘部法则最佳 K 值
...
def main():
    ...
    plot_cluster_scatter(df, save_path='cluster_scatter_plot_no_timeslot.png')

    # 绘制每个聚类类别的雷达图
    plot_heatmap(df, features_to_normalize, title='Cluster Behavior Analysis', save_path='cluster_heatmap_no_timeslot.png')
